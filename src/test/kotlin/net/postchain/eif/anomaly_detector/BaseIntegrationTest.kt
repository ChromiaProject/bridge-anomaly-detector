...
abstract class EifBaseIntegrationTest(private val prependUrls: List<String> = listOf()) : ManagedModeTest() {
    ...
    protected val evmContainer: DockerComposeContainer<*> =
    ...
    open fun setup() {
        ...
        evmRpcUrl = getRpcUrl()
        web3j = Web3j.build(HttpService(evmRpcUrl))
        ...
        var urls = mutableListOf(evmRpcUrl)
        urls.addAll(prependUrls)
        with(configOverrides) {
            setProperty("ethereum.urls", urls.joinToString(","))
        }
    }
